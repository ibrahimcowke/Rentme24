    ...
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-700 pb-12">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-black tracking-tight">Finance <span className="text-indigo-600 italic">Core</span></h1>
          </div>
          <p className="text-slate-500 font-medium">Advanced financial analytics and performance reporting.</p>
        </div>
        
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 px-5 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl hover:bg-slate-50 transition-all font-bold text-sm">
            <Calendar size={18} />
            <span>Last 6 Months</span>
          </button>
          <button className="flex items-center gap-2 px-6 py-2.5 bg-indigo-600 text-white rounded-2xl hover:bg-indigo-700 transition-all font-bold shadow-lg shadow-indigo-500/20">
            <Download size={18} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {[
          ...
          >
             ...
             <h3 className="text-4xl font-black tracking-tighter mb-2">{card.value}</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="glass-card p-8 rounded-[2.5rem] border border-white/20 dark:border-slate-800/50 shadow-2xl relative overflow-hidden">
          ...
          <h3 className="text-xl font-black mb-10 italic">Revenue Velocity</h3>
           ...
           <div className="h-[350px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                 <BarChart data={revenueData.slice(2)}>
                    ...
                    <Tooltip cursor={{ fill: 'transparent' }} contentStyle={{ backgroundColor: '#1E293B', border: 'none', borderRadius: '16px' }} />
                 </BarChart>
              </ResponsiveContainer>
           </div>
        </div>
         ...
         <div className="lg:col-span-2 glass-card p-8 rounded-[2.5rem] border border-white/20 dark:border-slate-800/50 shadow-2xl">
            ...
            <div className="space-y-4">
               {[
                  ...
                  <div key={i} className="flex items-center justify-between p-4 bg-slate-100/50 dark:bg-slate-800/30 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 group hover:shadow-lg transition-all cursor-pointer">
                     <div className="flex items-center gap-4">
                        <div className="p-3 bg-indigo-500/10 text-indigo-600 rounded-xl group-hover:bg-indigo-600 group-hover:text-white transition-all">
                           <FileText size={20} />
                        </div>
                        <div>
                           <p className="text-sm font-black group-hover:text-indigo-600 transition-colors uppercase tracking-tight">{report.name}</p>
                           <p className="text-[10px] font-bold text-slate-400">{report.size} • {report.date}</p>
                        </div>
                     </div>
                     <button className="p-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl hover:bg-indigo-600 hover:text-white transition-all shadow-sm">
                        <Download size={18} />
                     </button>
                  </div>
               ))}
